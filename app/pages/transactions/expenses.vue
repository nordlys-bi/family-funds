...
const saveTransaction = async () => {
  ...
  try {
    ...
    notice.value = {
      severity: 'success',
      text: isEdit ? 'Ausgabe wurde aktualisiert.' : 'Ausgabe wurde angelegt.',
    }
  } catch (error: any) {
    notice.value = {
      severity: 'error',
      text: 'Ausgabe konnte nicht gespeichert werden: ' + (error.statusMessage || error.message),
    }
  } finally {
...
const deleteTransaction = async (transaction: TransactionItem) => {
  ...
  try {
    ...
    notice.value = {
      severity: 'success',
      text: 'Ausgabe wurde gelöscht.',
    }
  } catch (error: any) {
    notice.value = {
      severity: 'error',
      text: 'Ausgabe konnte nicht gelöscht werden: ' + (error.statusMessage || error.message),
    }
  } finally {
  ...
  >
    ...
    <section v-else-if="!activeHousehold" class="empty-state">
      <div class="empty-state__card">
        ...
        <p>Erst dann können wir Ausgaben erfassen.</p>
    ...
    <section v-else class="list-panel">
      <div class="list-panel__head">
        ...
        <span class="panel-badge">{{ formatMoney(summary.expenseTotal) }}</span>
      </div>
        ...
        <article v-for="transaction in visibleTransactions" :key="transaction.id" class="item-card">
          <div class="item-main">
            <div class="item-title-row">
              ...
              <Tag severity="info" :value="formatMoney(transaction.amount)" class="item-pill" />
            </div>
            <div class="item-meta-row">
              <Tag severity="secondary" :value="budgetLabel(transaction)" class="item-tag" />
            </div>
    ...
    >
      <form class="transaction-dialog" @submit.prevent="saveTransaction">
        ...
        <div class="dialog-actions">
          ...
          <Button
            type="submit"
            :label="editingTransactionId ? 'Ausgabe aktualisieren' : 'Ausgabe anlegen'"
            icon="pi pi-check"
